...
typedef enum {
    ...
    OPERAND_MEMORY,
    OPERAND_STACK
} _UNIT_AMD64_OperandKind;

typedef struct {
    _UNIT_AMD64_OperandKind kind;

    union {
        _UNIT_AMD64_Register reg;
        uint64_t immediate;
    };
} _UNIT_AMD64_Operand;

typedef struct {
    _UNIT_AMD64_Opcode opcode;
    _UNIT_AMD64_Operand operands[2];
    UNIT_Size operand_count;
} _UNIT_AMD64_Instruction;

UNIT_Status
_UNIT_AMD64_encode_instruction(_UNIT_CompileContext *context,
